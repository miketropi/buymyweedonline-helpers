...
function b_helpers_free_gift() {
  ...
  if(empty($freegift_products)) return;
...
function b_helpers_get_the_rest_of_amount() {
  ...
  if(!is_array($freegift_products)) return;
...
function b_helpers_meta_tag_after_button_mini_cart() {
  ...
  if(empty($after_button)) return;
  if(!empty($after_button) && !isset($after_button['enable']) && $after_button['enable'] != true) return;
